...
LPBITMAPINFOHEADER lpbi;
    ...
    pBuf = (LPSTR)lpbi + (WORD)lpbi->biSize + PaletteSize(lpbi);
	if( dx < dx0 || dy < dy0)
		SetStretchBltMode(hdc, HALFTONE); //smoothing when  shrinking, to prevent dark stripes in COLORONCOLOR mode.
	else
		SetStretchBltMode(hdc, COLORONCOLOR); //no smoothing when enlarging
...
HANDLE CDIB::DibFromDC (HDC hDC, int x0, int y0, int w, int h)
{
	HANDLE hDib;
	HBITMAP hBmp	= this->GetSrcBit(hDC,(DWORD)x0, (DWORD)y0, (DWORD)w, (DWORD)h);
	hDib			= this->DibFromBitmap(hBmp, BI_RGB, 24, NULL);
	return hDib;
}

HBITMAP CDIB::GetSrcBit (HDC hDC, DWORD x0, DWORD y0, DWORD BitWidth, DWORD BitHeight)
{

// Source: http://www.cppblog.com/justin-shi/archive/2008/09/28/62991.html
		HDC hBufDC;
		...
		StretchBlt(	hBufDC, 0, 0, BitWidth, BitHeight,
					hDC, x0, y0, BitWidth, BitHeight, SRCCOPY);
		//得到最终的位图信息
